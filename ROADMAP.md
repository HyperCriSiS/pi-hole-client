...
- [ ] Keep `/api/history/clients` on the handwritten client until `PiholeV6Service.getHistoryClients` forwards the existing `N`/count parameter; generated response-model parity is otherwise verified, so fix parameter forwarding separately before migrating the repository call.
