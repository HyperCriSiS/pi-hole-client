...
- [ ] #639 / #352: incrementally improve Pi-hole v6 API support and server-side log filtering with parity tests; retain handwritten transport only for explicit compatibility/schema/behavior gaps that the generated client cannot yet preserve.
  - [x] Model the FTL-supported server-side query filters (`domain`, `client_ip`, `client_name`, `status`, `type`, `reply`) as `V6QueryFilter`, including normalization and empty-value handling.
  ...
  - [x] Push exactly one active client selection server-side: valid IPv4/IPv6 literals use `client_ip`, while a hostname uses FTL's exact `client_name` parameter. The all-selected state and multi-client selections remain client-side; focused model/repository/ViewModel regression coverage preserves that boundary.
...
- [x] Migrate `/api/action/flush/network` with the existing HTTP 404 fallback to deprecated `/api/action/flush/arp` onto generated actions; preserve shared SID injection, 401 renewal/retry behavior and pre-v6.3 compatibility.
- [ ] Keep gravity update handwritten for now because the current client exposes the streaming progress contract while the generated endpoint does not.
