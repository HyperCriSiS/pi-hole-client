...
- [ ] Keep `/api/queries` on the handwritten client for now: the repository contract exposes legacy `start` pagination in addition to cursor pagination, while the generated `getQueries` wrapper does not expose `start`; migrate only after that behavior can be preserved explicitly.
