...
| #352 | v6 server-side log filtering | **Partially implemented in `main`.** v6 query-log requests push semantically safe exact filters server-side: domain, one concrete status, and exactly one active client selection (`client_ip` for IPv4/IPv6 literals or `client_name` for a hostname). Multi-status/grouped selections, multiple/all-selected clients, and UI filters without an equivalent FTL parameter remain local to preserve semantics; v5 remains fully local. Keep open only for the remaining bounded filter cases. |
