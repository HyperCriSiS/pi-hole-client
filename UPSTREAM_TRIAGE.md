...
| #570 | Local CNAME | **Implemented and CI-validated in `main` for Pi-hole v6.** Added verified CNAME domain/repository CRUD with optional TTL preservation plus a capability-gated Host/CNAME Local DNS UI. Reads and list-replacement updates now use generated config APIs; add/delete intentionally remain handwritten until generated path encoding is proven equivalent. Pi-hole v5 remains unchanged because its Local DNS API path does not expose CNAME support. |
