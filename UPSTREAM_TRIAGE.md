...
|---|---|---|
| #639 | Pi-hole v6 API | **In progress in `main`.** Generated-client migration covers the parity-verified info paths including `/api/info/client`, history/stats, `/api/queries` with legacy `start` pagination preserved, network-device reads/deletion, DNS actions, DHCP, DNS blocking, DNS query-logging config reads, Local DNS/CNAME config reads/updates, and authenticated auth/session reads/deletes. The generated client is reproducibly refreshed to pinned FTL v6.7 (`fa65a88f8cdef1013594d4de14108077954faea4`) with pinned Redocly/OpenAPI Generator tooling, and the remaining handwritten paths were re-audited against that schema. Keep `/api/info/ftl`, session creation/renewal `POST /auth`, the unauthenticated auth capability probe, detailed network gateway, ARP/network fallback flushing, gravity streaming, and Local DNS add/delete handwritten until their explicit compatibility/schema/behavior/encoding gaps are resolved. Local DNS add/delete is now a demonstrated encoding mismatch: generated path placeholders are substituted raw while the proven handwritten client percent-encodes slash-containing element/value segments. |
...
| #570 | Local CNAME | **Implemented and CI-validated in `main` for Pi-hole v6.** Added verified CNAME domain/repository CRUD with optional TTL preservation plus a capability-gated Host/CNAME Local DNS UI. Reads and list-replacement updates use generated config APIs; add/delete intentionally remain handwritten because generated v6.7 substitutes config path placeholders raw, whereas the existing client percent-encodes slash-containing elements/values (`dns/hosts` -> `dns%2Fhosts`). Pi-hole v5 remains unchanged because its Local DNS API path does not expose CNAME support. |
...
6. #639/#352: incremental generated-v6 migration remains active on `main`; #352 now includes exact single-hostname transport via `client_name` in addition to domain/status/IP filters, the generated snapshot is reproducibly pinned to FTL v6.7, and the remaining handwritten paths are explicit compatibility/schema/behavior/encoding holds (including the proven Local DNS path-encoding mismatch). #570 is complete. #134 source-build work is CI-validated but downstream submission remains blocked on the required independent-fork package identity.
