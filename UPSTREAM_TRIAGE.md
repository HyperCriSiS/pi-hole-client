...
6. #639/#352: incremental generated-v6 migration remains active on `main`; #352 now includes exact single-hostname transport via `client_name` in addition to domain/status/IP filters, the generated snapshot is reproducibly pinned to FTL v6.7 with deterministic formatting and URI-component-safe generated path parameters, and the remaining handwritten paths are explicit compatibility/schema/behavior holds. #570 is complete with generated v6 Local DNS/CNAME CRUD. #134 source-build work is CI-validated but downstream submission remains blocked on the required independent-fork package identity.
