...
| #352 | v6 server-side log filtering | **Partially implemented in `main`.** v6 query-log requests push semantically safe exact filters server-side (domain, one concrete status, and one literal client IP). Multi-status/grouped selections, hostnames/multiple clients, and UI filters without an equivalent FTL parameter remain local to preserve semantics; v5 remains fully local. Keep open only for the remaining bounded filter cases. |
...
6. #639/#352: incremental generated-v6 migration remains active on `main`; #352 is partially implemented for semantically safe v6 filters, the generated snapshot is reproducibly pinned to FTL v6.7, and the remaining handwritten paths are explicit compatibility/schema/behavior/encoding holds. #570 is complete. #134 source-build work is CI-validated but downstream submission remains blocked on the required independent-fork package identity.
